...
export function localStorageMonthlyTopicOrderKey(userId: string | null, year: number, month: number) : string {
  if (userId) {
...
export function localStorageMonthData(userId: string | null, year: number, month: number) : MonthData {
  return {
...
export function localStorageMonthlyTopicOrder(userId: string | null, year: number, month: number) : Array<number> {
...
export function localStorageSetMonthlyTopicOrder(userId: string | null, year: number, month: number, topicOrder: Array<number>) {
  const topicOrderStorageKey = localStorageMonthlyTopicOrderKey(userId, year, month);
  localStorage.setItem(topicOrderStorageKey, JSON.stringify(topicOrder));
}

export function localStorageSetMonthlyTopics(userId: string | null, year: number, month: number, topics: Array<MonthlyTopicData>) {
  for (let topicId = 0; topicId < topics.length; topicId++) {
    localStorageSetMonthlyTopicData(userId, year, month, topicId, topics[topicId]);
  }
}

export function localStorageSetMonthData(userId: string | null, year: number, month: number, monthData: MonthData) {
...
export function localStorageClearMonthData(userId: string | null, year: number, month: number) {
